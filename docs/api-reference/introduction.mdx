Welcome to the Tartarus API! 

Designed for seamless integration, scalability, and security, it offers a powerful interface to manage configurations efficiently for any application.

## Authorization

Tartarus APIs require authentication to ensure secure access. Authentication is handled via an API Key, which must be included in every request.

The API Key is included in every request as

```bash
Authorization: Bearer <YOUR_API_KEY>
```

## API Request and Response

<Icon icon="scroll-old" /> **Use Case:** \
For *listing config_definitions*, with query parameters:

| **Parameter** | **Type** | **Description**                                      |
|---------------|----------|------------------------------------------------------|
| `page`        | `int`    | The page number.                                     |
| `limit`       | `int`    | The limit for the number of configuration definitions to return. |
| `sort_by`     | `str`    | The field to sort by.                                |
| `sort_order`  | `str`    | The order to sort by.                                |
| `search`      | `str`    | The search term.                                     |


<Accordion title="API Request">
```bash
curl -X 'GET' \
  'http://127.0.0.1:8000/api/v1/config_definition/?page=1&limit=10&sort_by=modified_at&sort_order=desc' \
  -H 'accept: application/json' \
  -H 'Authorization: <YOUR_API_KEY>'
```
</Accordion>

<Accordion title="API Response">
```json
{
  "message": "Configuration definitions listed successfully.",
  "data": {
    "results": [],
    "meta": {
      "page": 1,
      "limit": 10,
      "total": 0
    }
  }
}
```
</Accordion>

## Interactive API Testing with Swagger

Tartarus comes with **Swagger UI** to explore and test API endpoints effortlessly.

Visit [`http://localhost:8000/docs`](http://localhost:8000/docs) after starting your Tartarus server.