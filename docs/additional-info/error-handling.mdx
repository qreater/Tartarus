...
Each error in Tartarus is mapped to a specific `JSON` response, ensuring consistency and clarity. These responses are customised based on use-cases
...
A dedicated middleware intercepts unhandled errors and formats them into a consistent `JSON` structure. This ensures that even unexpected issues are presented in a user-friendly format. 
...
|------------------|-------------------------------|---------------------------------------------------------------------------------|
| **409 Conflict** | `ConflictError`              | Occurs when a request conflicts with the current state of the server (e.g., duplicate resource). |
| **404 Not Found**| `ResourceNotFoundError`      | The requested resource does not exist on the server.                           |
| **422 Unprocessable Entity** | `ValidationError`| The request contains invalid or malformed data that cannot be processed.       |
| **401 Unauthorized** | `UnauthorizedError`      | Authentication credentials are missing or invalid.                             |
| **500 Internal Server Error** | `InternalError` | An unexpected server error occurred. This is usually not the client’s fault.   |
